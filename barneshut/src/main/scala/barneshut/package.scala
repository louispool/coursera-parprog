...
package object barneshut {
  ...
  /**
   * Quad represents a square cell of space, and can be one of the following node types:
   */
  sealed abstract class Quad {
    ...
    //total mass of all bodies in the cell
    def mass: Float
    ...
    //the length of one side of the cell
    def size: Float
  ...
  /**
   * Empty node, which represents an empty quadtree
   */
  ...
  /**
   * Fork node, which divides a spatial cell into four quadrants
   */
  case class Fork(nw: Quad, ne: Quad, sw: Quad, se: Quad) extends Quad {
    ...
    def insert(b: Body): Fork = {
      ...
      Fork(if (b.x <= centerX && b.y <= centerY) nw.insert(b) else nw,
           if (b.x  > centerX && b.y <= centerY) ne.insert(b) else ne,
           if (b.x <= centerX && b.y  > centerY) sw.insert(b) else sw,
           if (b.x  > centerX && b.y  > centerY) se.insert(b) else se)
    }
  }

  /**
   * Leaf node, which represents one or more bodies
   */
  case class Leaf(centerX: Float, centerY: Float, size: Float, bodies: Seq[Body]) extends Quad {

    val mass  = bodies.foldLeft(0f)(_ + _.mass)

    val massX = bodies.foldLeft(0f)((z, b) => z + b.mass*b.x) / mass
    val massY = bodies.foldLeft(0f)((z, b) => z + b.mass*b.y) / mass
    ...
    def updated(quad: Quad): Body = {
      ...
      def traverse(quad: Quad): Unit = (quad: Quad) match {
        ...
        case Fork(nw, ne, sw, se) =>
          ...
          def check(quad: Quad) = if (quad.size / distance(quad.massX, quad.massY, x, y) < theta) traverse(quad)
        ...
        if (span == 1) {
          ...
          val (nw, ne, sw, se) =
            if (parallelism > 1 && achievedParallelism < parallelism * BALANCING_FACTOR) {
              parallel(quad(x, y, nspan, nAchievedParallelism),
